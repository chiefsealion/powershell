...
function RunADAudit-AdminAccountReview
{
    get-adobject -ldapfilter "(admincount=1)" | Export-CSV C:\ADReports\AdminAccountsReview.CSV
}

function RunADAudit-PWNeverExpiresReport
{
    Get-AdUser -filter { passwordNeverExpires -eq $true  -and enabled -eq $true } | Select Name, DistinguishedName | export-csv c:\adreports\PWNeverExpiresReport.CSV
}

#endregion
#region Remediation Functions

function Remediate-InactiveMachines

{
    $csv = import-csv c:\adremediations\90DAY_InactiveMachines.CSV
    $csv | foreach{Disable-ADAccount -Identity $_.Name}
}

function Remediate-InactiveUsers
{
    $csv = import-csv c:\adremediations\90DAY_InactiveUserAccounts.CSV
...
function Remediate-KerberosTGTReset
